...
Measured idle RSS with the same config: 2026.7.1-2 on Node 22 is 275 MB, on Node 24 is 377 MB, and 2026.9.4 on Node 24 is 432 MB. About two thirds of the growth is the Node bump, which 2026.9.4 forces (engines >=24.16), so pinning OpenClaw alone does not recover it. Nothing turns it down: plugins.allow does not reduce the loaded set (30 of 59 plugins stay enabled), and capping the V8 heap buys about 10 MB before the process fails to start. At a 480m limit the gateway idles at 90% and restarts itself every ~73s under its own memory-pressure check, while each boot passes the health check. `npm audit` reports 11 advisories against 2026.7.1-2 (7 high) and none against 2026.9.4. Reachability here is low: no published port, no web tools under tools.allow, one allowlisted sender.
...
Take OpenClaw 2026.9.4 on Node 24. GATEWAY_MEM_LIMIT defaults to 1g, and that is a floor. A deployment that genuinely needs 1 GB should pin the old version deliberately, knowing what it accepts, rather than drift into it.
