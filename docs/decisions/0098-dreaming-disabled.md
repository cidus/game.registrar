...
OpenClaw's memory-core ships a `dreaming` sweep enabled by default. Nightly, it consolidates memory from session corpora and has a model write a narrative entry into DREAMS.md in the workspace, which is compiled into the system prompt of every later turn, growing each night. README reports two nights came to 3,800 bytes. `tools.allow` is exec/message/read, so the agent has no tool to query what the sweep built, and the diary contradicts the standing order 'no notes, no session history; the register is the memory'. The live gateway also held OpenClaw's generic USER.md, so the repo's prompt-size budget measured only a subset.
...
The boot overlay sets `plugins.entries.memory-core.config.dreaming.enabled: false` on every boot. An existing workspace DREAMS.md is moved to /config/DREAMS.md (timestamped if one exists), never deleted. USER.md and HEARTBEAT.md ship from the repo so the deployed set equals the shipped set, and the size budget covers the whole prompt.
...
No memory consolidation. The old diary survives outside the prompt. Because the overlay reapplies `enabled: false` on every boot, re-enabling it in the saved config is reverted at the next restart; truly re-enabling needs an entrypoint change. Lesson: a budget over a subset is not a budget.
