...
On OpenClaw 2026.7.1-2 a sticker is a channel action, not a presentation block: MessagePresentationBlock is text | context | divider | buttons | select. So a reaction cannot ride along with a reply the way an inline keyboard does, and the config schema has no slot for a token table. `sendSticker` (to, fileId) is gated by channels.telegram.actions.sticker and throws when off. `react` (messageId, emoji) is gated by actions.reactions and by reactionLevel above 'off', returning {ok:false, reason:'disabled'}. A Telegram file_id belongs to one bot. The spec originally said the mapping 'lives in the user's config' the project's working notes.
...
A reaction is a second message-tool call made after the command returns. The token-to-asset table is agent/workspace/REACTIONS.md: gateway side, per installation, user-owned. No artwork ships. Both gateway switches ship commented out. The five tokens (filed, approved, archived, pending, puzzled) are identifiers, never translated.
...
Nothing happens until an operator enables the switches. The sticker column must be rebuilt after replacing the bot. A translated token matches no row and silently does nothing, indistinguishable from an empty installation, which is why the warning is repeated in several files. At most one reaction per turn and none on a check-in wake.
