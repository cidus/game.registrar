...
`cron add` connects over the WebSocket to a running gateway, so the entrypoint cannot run it before exec'ing the gateway, which is where it seemed to belong. OpenClaw's config schema has no `cron.jobs` array; the job store is reachable only through the CLI against a live gateway. Reaching the gateway over the compose network as ws://gateway:18789 is refused: OpenClaw rejects plaintext ws:// to a non-loopback address.

`openclaw cron add` is a Gateway client command, so the entrypoint cannot register the check-in job before exec'ing the gateway. OPENCLAW_STATE_DIR and OPENCLAW_CONFIG_PATH exist, so one mounted directory can hold config, workspace, allowlist, cron store and transcripts.
...
A one-shot `provision` service gated on gateway health registers the cron job idempotently. All gateway state lives under one /config mount.
...
Host installs still register the job by hand (README step 8), and a future generator must do the same for non-image installs. provision restarts on-failure up to 5 times. Without the gateway token it fails with GatewayCredentialsRequiredError. `provision --dry-run` must not contact the gateway.
