...
- A checkout of this repository with `npm install` completed, for
  `scripts/vendor-quartz.sh` and its YAML parser. Nothing else in the container
  installation needs a checkout.
...
scripts/vendor-quartz.sh --clone
```

Use a Quartz v5 checkout with npm plugin support. The `v5.0.0` tag predates
that support, even though later compatible checkouts still declare version
`5.0.0` in `package.json`. With no `--tag`, the script fetches the upstream
default branch. Use `--tag` only with a compatible tag or branch you have
verified; a local checkout pinned to a commit can be passed via `--source`.
See [ADR 0103](../decisions/0103-quartz-bootstrap-needs-compatible-code-and-theme.md).
...
survives an update. The enabled `@quartz-themes/core` plugin’s theme is also
added as a dependency before installation (a variation uses its base package).
Existing version constraints are preserved. `package-lock.json` is deleted and
regenerated by `npm install`, which resolves those extra entries.
